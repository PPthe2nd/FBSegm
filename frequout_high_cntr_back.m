...
%     clear eh name pippo genny bin_1 deh rsm_orig amin amax
    ...
    pippo = Shape.(name);
    genny = Berk.(name);
    bin_1 = 1-pippo; % 1- pippo = masking foreground
    deh = genny.*(pippo);
...
%         clear b bin_masked rsm_new rsm_fin Img_bin_1 Img_single_bin_1 p_bin_1 f_bin_1 d_bin_1 l_bin_1 g_bin_1 
        ...
        b = imagefilter(genny,constructbutterfilter(500, [ranges(j) 500],5)); % [1 ranges(j)] = add high freq; [ranges(j) 500] = add low freq
...
for i = 1:6
    ...
    feats_high_back{i} = n_app_corr_bin_1;

end

save('feats_high_back','feats_high_back');
